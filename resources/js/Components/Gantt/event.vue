<template>
    <div
        ...
        ref="vuecalEvent"
        class="vuecal__event cursor-pointer"
        :class="eventClasses"
        :style="eventStyles"
        tabindex="0"
        @focus="focusEvent"
    ...
    >
        <div
            ...
            @touchstart.stop="touchDeleteEvent"
        >
            {{ vuecal.texts.deleteEvent }}
        </div>
        <slot name="event" :event="event" :view="view.id"></slot>
        <div
    ...
    <p
        ...
        "
    >
        {{ grabRelationshipInfo }}
    </p>
    <Modal
        title="Add Relationship"
        width="50%"
        v-if="toggleRelationshipModal"
        @toggleModal="toggleRelationshipModal = $event"
    >
        <template #body>
            <div
                v-for="(relationship, index) in event.relationships"
                :key="'relationship-' + index"
                class="ml-6"
                style="display: grid; grid-template-columns: 45% 45% 10%"
            >
                <select-input
                ...
                <font-awesome-icon
                    ...
                    class="pr-6 cursor-pointer self-center"
                    icon="fa-solid fa-trash"
                />
            </div>
            <div class="flex flex-wrap ml-3 mb-2">
                <button
                    type="button"
                    class="secondary-btn"
...
export default {
    ...
    props: {
        ...
        cell: { type: Object, default: () => ({}) }, // Is the event displayed in the all-day bar.
    ...
    watch: {
        "event.relationships": {
            handler: function () {
                this.relationships = [
                    ...this.event.relationships.filter(
                        (relationship) =>
                            relationship.type && relationship.taskId
                    ),
                ];
                this.$nextTick(() => {
                    this.$refs.blockText.style.left =
                        -1 *
                            (this.$refs.blockText.getBoundingClientRect()
                                .width +
                                30) +
                        "px";
                });
    ...
    mounted() {
        if (!this.event.relationships?.length)
            this.event.relationships.push({
                type: "",
                taskId: "",
            });
        const resource = this.resources.find(
            (resource) => resource.id == this.event.resourceId
        );
        this.events = (resource?.events ?? []).filter(
            (event) => event.id != this.event.id
        );
        this.originalOffsetWidth = this.$refs.vuecalEvent.offsetWidth;
        this.arrowRightVal =
            Object.keys(this.event.taskHours).length * 100 - 84;
        this.$refs.resizerArrowRight.$el.style.right =
            -1 * this.arrowRightVal + "%";
        this.arrowLeftVal = Object.keys(this.event.taskHours).length * 5;
        this.$refs.resizerArrowLeft.$el.style.left =
            -1 * this.arrowLeftVal + "%";
        this.chunkSize =
            this.originalOffsetWidth / Object.keys(this.event.taskHours).length;
        this.oldVal = this.originalOffsetWidth;
        document.addEventListener("mouseup", (e) => {
            this.isResizingLeft = false;
            this.isResizingRight = false;
            this.oldVal = this.originalOffsetWidth;
            if (this.$refs.vuecalEvent)
                this.$refs.vuecalEvent.style.left = "0px";
            if (this.$refs.resizerArrowRight) {
                this.$refs.vuecalEvent.style.width =
                    100 * Object.keys(this.event.taskHours).length + "%";
                this.arrowRightVal =
                    Object.keys(this.event.taskHours).length * 100 - 84;
                this.$refs.resizerArrowRight.$el.style.right =
                    -1 * this.arrowRightVal + "%";
            }
            if (this.$refs.resizerArrowLeft) {
                this.arrowLeftVal =
                    Object.keys(this.event.taskHours).length * 5;
                this.$refs.resizerArrowLeft.$el.style.left = "-15%";
                this.$refs.blockText.style.left =
                    -1 *
                        (this.$refs.blockText.getBoundingClientRect().width +
                            30) +
                    "px";
            }
            this.newlyAddedEventDays = 0;
        });
        document.addEventListener("mousemove", (e) => {
            if (this.isResizingRight) {
                if (this.$refs.resizerArrowRight.$el && this.$refs.vuecalEvent)
                    this.$refs.resizerArrowRight.$el.style.right =
                        -1 * this.arrowRightVal -
                        (e.clientX - this.dragStartPoint) +
                        "%";
                this.$refs.vuecalEvent.style.width =
                    this.oldVal + (e.clientX - this.dragStartPoint) + "px";
                if (
                    this.originalOffsetWidth + this.chunkSize <=
                    this.$refs.vuecalEvent.offsetWidth
                ) {
                    this.$emit("addEvent", {
                        cell: this.cell,
                        event: this.event,
                    });
                    this.originalOffsetWidth =
                        this.originalOffsetWidth + this.chunkSize;
                } else if (
                    this.originalOffsetWidth - this.chunkSize >=
                    this.$refs.vuecalEvent.offsetWidth
                    ...
                    this.$refs.resizerArrowLeft.$el.style.left =
                        -1 *
                            (this.arrowLeftVal -
                                (e.clientX - this.dragStartPoint)) +
                        "%";
                this.$refs.blockText.style.setProperty(
                    "left",
                    `calc(${
                        -1 *
                            (this.$refs.blockText.getBoundingClientRect()
                        ...
                        this.arrowLeftVal - (e.clientX - this.dragStartPoint)
                    }px)`
                );
                if (this.$refs.resizerArrowLeft)
                    this.$refs.vuecalEvent.style.left =
                        -1 * (this.dragStartPoint - e.clientX) + "px";
                if (
                    (-1 * this.$refs.vuecalEvent?.offsetLeft ?? 0) >
                        this.chunkSize &&
                    !(
                        this.event.startDate.subtractDays(1) <
                        new Date().setHours(0, 0, 0, 0)
                    )
                ) {
                    this.$emit("addEventReverse", {
                        cell: this.cell,
                        event: this.event,
                    });
                } else if (
                    (this.$refs.vuecalEvent?.offsetLeft ?? 0) > this.chunkSize
                ) {
                    this.$emit("removeEventReverse", {
                        cell: this.cell,
                        event: this.event,
                    });
                }
    ...
    methods: {
        mouseover() {
            this.$refs.blockText.style.display = "block";
            this.$nextTick(() => {
                this.$refs.blockText.style.left =
                    -1 *
                        (this.$refs.blockText.getBoundingClientRect().width +
        ...
        removeRelationship(index) {
            const relationship = this.event.relationships.splice(index, 1);
            this.deletedRelationships.push(...relationship);
        },
        async saveRelationship() {
            this.relationships = this.event.relationships.filter(
                (relationship) => relationship.type && relationship.taskId
            );
            ...
            this.relationships.forEach(async (relationship) => {
                const relationshipAddedToTask = events.find(
                    (event) => event.id == relationship.taskId
                );
                this.event.relationships = [...this.relationships];
                this.event.employeeId = this.event.resourceId;
                this.event.plannedStartDate =
                    this.event.startDate.format("YYYY-MM-DD");
                this.event.plannedFinishedDate =
                    this.event.endDate.format("YYYY-MM-DD");
                try {
                ...
                const relationshipAddedToTaskIndex = events.findIndex(
                    (event) => event.id == relationship.taskId
                );
                if (
                    !relationshipAddedToTask.relationships.some((rs) =>
                        (rs.type == relationship.type) === "blocks"
                            ? "blockedBy"
                            : "blocks" && rs.taskId == this.event.id
                    )
                ) {
                    relationshipAddedToTask.relationships = [
                        ...relationshipAddedToTask.relationships.filter(
                            (relationship) =>
                                relationship.type && relationship.taskId
                        ),
                    ];
                    relationshipAddedToTask.relationships.push({
                        type:
                            relationship.type === "blocks"
                                ? "blockedBy"
                                : "blocks",
                        taskId: this.event.id,
                    });
                    events[relationshipAddedToTaskIndex] = {
                        ...relationshipAddedToTask,
                    };
                    relationshipAddedToTask.employeeId =
                        relationshipAddedToTask.resourceId;
                    relationshipAddedToTask.plannedStartDate =
                        relationshipAddedToTask.startDate.format("YYYY-MM-DD");
                    relationshipAddedToTask.plannedFinishedDate =
                        relationshipAddedToTask.endDate.format("YYYY-MM-DD");
                    try {
                        await this.$store.dispatch("tasks/update", {
                            id: relationshipAddedToTask.id,
                            data: { ...relationshipAddedToTask },
                        });
                        await this.$store.dispatch("tasks/list");
                        await this.$store.dispatch(
                        "projectManagement/projectData",
            ...
            this.deletedRelationships.forEach(async (relationship) => {
                const relationshipRemovedFromTask = events.find((event) => {
                    return event.id == relationship.taskId;
                });
                const relationshipIndex =
                    relationshipRemovedFromTask.relationships.findIndex((r) => {
                        return (
                            r.type == relationship.type &&
                            e.taskId == relationship.taskId
                        );
                    });
                relationshipRemovedFromTask.relationships.splice(
                    relationshipIndex,
                    1
                );
                relationshipRemovedFromTask.employeeId = this.event.resourceId;
                relationshipRemovedFromTask.plannedStartDate =
                    relationshipRemovedFromTask.startDate.format("YYYY-MM-DD");
                relationshipRemovedFromTask.plannedFinishedDate =
                    relationshipRemovedFromTask.endDate.format("YYYY-MM-DD");
                try {
                    await this.$store.dispatch("tasks/update", {
                        id: relationshipRemovedFromTask.id,
                        data: { ...relationshipRemovedFromTask },
                    });
                    await this.$store.dispatch("tasks/list");
                    await this.$store.dispatch(
                        "projectManagement/projectData",
        ...
        onMouseLeave(e) {
            this.$refs.blockText.style.display = "none";
            e.preventDefault();
        ...
        onDragStart(e) {
            this.dragging = true;
            this.innerHtmlHours = this.$refs.vuecalEvent.innerHTML;
            this.$refs.vuecalEvent.innerHTML = `<p style="font-size: 0.7rem;">${this.event.startDate
                .toString()
                .slice(0, 10)} - ${this.event.endDate
                .toString()
                .slice(0, 10)}</p>`;
            e.dataTransfer.setData("eventId", this.event.id);
            e.dataTransfer.setData("resourceId", this.event.resourceId);
        ...
        onDragEnd() {
            this.dragging = false;
            if (this.$refs.vuecalEvent)
                this.$refs.vuecalEvent.innerHTML = this.innerHtmlHours;
    ...
    computed: {
        grabRelationshipInfo() {
            ...
            for (let i = 0; i < this.relationships.length; i++) {
                relationshipText += `${
                    this.enums[this.relationships[i].type]
                } ${
                    this.events.find(
                        (event) => event.id == this.relationships[i].taskId
                    )?.name ?? ""
                }${this.relationships[i + 1] ? ", " : ""}`;
            }
            return relationshipText;
        },
        checkIfEventIsLastInRange() {
            let taskHours = Object.keys(this.event.taskHours);
            return (
                taskHours[taskHours.length - 1 - this.newlyAddedEventDays] ===
                this.cellFormattedDate
            );
        },
        eventDimensions() {
            const { startTimeMinutes, endTimeMinutes } =
            ...
            const top = Math.max(
                Math.round(
                    (minutesFromTop * this.vuecal.timeCellHeight) /
                        this.vuecal.timeStep
                ),
                0
            );
            ...
            // Bottom of event.
            minutesFromTop =
                Math.min(endTimeMinutes, this.vuecal.timeTo) -
...
.vuecal__event-title--edit {
    ...
    transition: 0.3s;
    color: inherit;
    background-image: url('data:image/svg+xml;utf8,<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="M442 150l-39 39-130-130 39-39q6-6 15-6t15 6l50 50q6 6 6 15t-6 15zM64 368l236-236 80 80-236 236H64v-130z" fill="%23000" opacity=".4"/></svg>');
