<template>
    ...
    >
        ...
        `"
            ...
            <div
                class="vuecal__flex vuecal__cell-content cursor-pointer"
                style="height: 15px"
                v-for="(split, i) in splitsCount
                    ? splits
                    : resource.rowExpanded
...
export default {
    ...
    methods: {
        ...
        checkIfInRange(startDate, endDate) {
                    ...
                    for (let i = 0; i < resource.events.length; i++)
                        resources.push({
                            ...
                            events: events.filter(
                                (event) => event.id == resource.events[i].id
                            ),
                        });
                }
            });
...
<style lang="scss">
.hover-check:hover .startConnector {
    display: block;
}
.hover-check:hover .endConnector {
    display: block;
}

.startConnector:hover {
