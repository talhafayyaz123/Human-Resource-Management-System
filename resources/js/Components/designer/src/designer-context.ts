...
export class DesignerContext {
    public static create(
        ...
        const isToolboxCollapsed = configuration.toolbox
            ? configuration.toolbox.isCollapsed ?? layoutController.isMobile()
            : false;
        const isEditorCollapsed = configuration.editors
            ? configuration.editors.isCollapsed ?? layoutController.isMobile()
            : false;
        ...
        const definitionWalker =
            configuration.definitionWalker ?? new DefinitionWalker();
        const definitionModifier = new DefinitionModifier(
            definitionWalker,
            state,
            configuration
        );
        ...
        if (configuration.undoStackSize) {
            historyController = HistoryController.create(
                configuration.undoStack,
                state,
                definitionModifier,
                configuration
            );
