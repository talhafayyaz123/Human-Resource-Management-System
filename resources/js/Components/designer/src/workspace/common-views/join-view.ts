...
export class JoinView {
    public static createStraightJoin(
        parent: SVGElement,
        start: Vector,
        height: number
    ) {
            ...
            default:
                {
                    ...
                    appendJoin(
                        parent,
                        `M ${f.x} ${f.y} q ${h * 0.3} ${h * -y * 0.8} ${h} ${
                            h * -y
                        } ` +
                            `l ${l.x - f.x - h * 2} 0 q ${h * 0.8} ${
                                -h * -y * 0.3
                            } ${h} ${-h * -y}`
                    );
...
function appendCurvedJoins(
    ...
    for (const target of targets) {
        ...
        appendJoin(
            parent,
            `M ${start.x} ${start.y} q ${x * h * 0.3} ${y * h * 0.8} ${x * h} ${
                y * h
            } ` +
