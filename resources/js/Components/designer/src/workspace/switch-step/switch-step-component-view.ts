...
export const createSwitchStepComponentViewFactory =
    ...
    (
        ...
        const branchComponents = branchNames.map((branchName) => {
            return viewContext.createSequenceComponent(
                g,
        ...
        const branchLabelViews = branchNames.map((branchName) => {
            const labelY =
                cfg.paddingTop + cfg.nameLabel.height + cfg.connectionHeight;
        ...
        const branchSizes = branchComponents.map((component, i) => {
            const halfOfWidestBranchElement =
                Math.max(branchLabelViews[i].width, cfg.minContainerWidth) / 2;

            const branchOffsetLeft =
                Math.max(halfOfWidestBranchElement - component.view.joinX, 0) +
                cfg.paddingX;
            const branchOffsetRight =
                Math.max(
                    halfOfWidestBranchElement -
                        (component.view.width - component.view.joinX),
                    0
                ) + cfg.paddingX;

            const width: number =
                component.view.width + branchOffsetLeft + branchOffsetRight;
        ...
        const totalBranchesWidth = branchSizes.reduce(
            (result, s) => result + s.width,
            0
        );
        const maxBranchesHeight = Math.max(
            ...branchComponents.map((s) => s.view.height)
        );

        const halfOfWidestSwitchElement =
            nameLabelView.width / 2 + cfg.paddingX;
        const switchOffsetLeft = Math.max(halfOfWidestSwitchElement - joinX, 0);
        const switchOffsetRight = Math.max(
            halfOfWidestSwitchElement - (totalBranchesWidth - joinX),
            0
        );

        const viewWidth =
            switchOffsetLeft + totalBranchesWidth + switchOffsetRight;
        const viewHeight =
            maxBranchesHeight +
            cfg.paddingTop +
            cfg.nameLabel.height +
            cfg.branchNameLabel.height +
            cfg.connectionHeight * 2;

        const shiftedJoinX = switchOffsetLeft + joinX;
        Dom.translate(nameLabelView.g, shiftedJoinX, 0);

        const branchOffsetTop =
            cfg.paddingTop +
            cfg.nameLabel.height +
            cfg.branchNameLabel.height +
            cfg.connectionHeight;
        ...
        branchComponents.forEach((component, i) => {
            ...
            const branchOffsetLeft =
                switchOffsetLeft +
                branchSize.offsetX +
                branchSize.branchOffsetLeft;

            Dom.translate(
                branchLabelViews[i].g,
            ...
            if (component.hasOutput && stepContext.isOutputConnected) {
                const endOffsetTopOfComponent =
                    ...
                    cfg.branchNameLabel.height +
                    cfg.connectionHeight +
                    component.view.height;
                const missingHeight =
                    viewHeight - endOffsetTopOfComponent - cfg.connectionHeight;
                if (missingHeight > 0) {
        ...
        JoinView.createJoins(
            ...
            branchSizes.map(
                (o) =>
                    new Vector(
                        switchOffsetLeft + o.offsetX + o.joinX,
                        cfg.paddingTop +
                            cfg.nameLabel.height +
                            cfg.connectionHeight
                    )
        ...
        if (stepContext.isOutputConnected) {
            ...
            if (ongoingJoinTargets.length > 0) {
                JoinView.createJoins(
                    g,
                    new Vector(shiftedJoinX, viewHeight),
                    ongoingJoinTargets
                );
            }
        }
        ...
        return {
            ...
            resolveClick(click: ClickDetails): true | null {
                return regionView.resolveClick(click) ||
                    g.contains(click.element)
                    ? true
                    : null;
            },
