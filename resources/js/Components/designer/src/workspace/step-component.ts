...
export class StepComponent implements Component {
    public static create(
        ...
        const badges = Badges.createForStep(
            stepContext,
            view,
            componentContext
        );
    ...
    public getPlaceholders(result: Placeholder[]) {
        if (!this.isDisabled) {
            if (this.view.sequenceComponents) {
                this.view.sequenceComponents.forEach((component) =>
                    component.getPlaceholders(result)
                );
            }
    ...
    public setIsDragging(isDragging: boolean) {
        if (!this.isDisabled) {
            if (this.view.sequenceComponents) {
                this.view.sequenceComponents.forEach((component) =>
                    component.setIsDragging(isDragging)
                );
            }
            if (this.view.placeholders) {
                this.view.placeholders.forEach((ph) =>
                    ph.setIsVisible(isDragging)
                );
            }
        }
