...
export const createTaskStepComponentViewFactory =
    ...
    (
        ...
        const settingsIcon = Dom.svg("image", {
            href: "../images/cog.svg",
            style: "cursor: pointer;",
        });
        const actionIcon = Dom.svg("image", {
            href: "../images/three-dots.svg",
            style: "cursor: pointer;",
        });
        Dom.attrs(settingsIcon, {
            height: "15",
            width: "15",
            x: boxWidth - 40,
            y: boxHeight / 10,
        });
        Dom.attrs(actionIcon, {
            height: "15",
            width: "15",
            x: boxWidth - 20,
            y: boxHeight / 10,
        ...
        g.insertBefore(settingsIcon, text);
        g.insertBefore(actionIcon, text);
        ...
        const isInputViewHidden =
            stepContext.depth === 0 &&
            stepContext.position === 0 &&
            !stepContext.isInputConnected;
        const isOutputViewHidden = isInterrupted;

        const inputView = isInputViewHidden
            ? null
            : InputView.createRoundInput(g, boxWidth / 2, 0, cfg.inputSize);
        const outputView = isOutputViewHidden
            ? null
            : OutputView.create(g, boxWidth / 2, boxHeight, cfg.outputSize);
