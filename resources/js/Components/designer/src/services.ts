...
function merge(services: Partial<Services>, extensions: DesignerExtension[]) {
    for (const ext of extensions) {
        ...
        if (ext.uiComponents) {
            services.uiComponents = (services.uiComponents || []).concat(
                ext.uiComponents
            );
        }
...
function setDefault(
    ...
    if (!services.stepComponentViewWrapper) {
        services.stepComponentViewWrapper =
            new DefaultStepComponentViewWrapperExtension();
    }
    ...
    if (configuration.editors) {
        services.uiComponents.push(
            new SmartEditorExtension(configuration.editors)
        );
    }
    ...
    if (!services.placeholderController) {
        services.placeholderController =
            new DefaultPlaceholderControllerExtension();
    }
    ...
    if (configuration.keyboard === undefined || configuration.keyboard) {
        services.daemons.push(
            KeyboardDaemonExtension.create(
                configuration.keyboard as
                    | true
                    | undefined
                    | KeyboardConfiguration
            )
