...
export class SmartEditorView {
    public static create(
        parent: HTMLElement,
        api: EditorApi,
        configuration: EditorsConfiguration
    ): SmartEditorView {
        ...
        // parent.prepend(root);
        if (
            (configuration as { globalEditorProvider?: object })
                .globalEditorProvider
        ) {
            throw new Error(
                "globalEditorProvider is renamed to rootEditorProvider"
            );
        }
    ...
    public setIsCollapsed(isCollapsed: boolean) {
        ...
        this.toggleIcon = Icons.createSvg(
            "sqd-smart-editor-toggle-icon",
            isCollapsed ? Icons.options : Icons.close
        );
