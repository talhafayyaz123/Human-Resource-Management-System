    ...
    >
        <div
            ...
            :key="
                ...
                data.formattedDate
            "
            style="height: 40px; border-top: 1px solid lightgrey"
        >
            <div
                style="
                    height: 40px;
            ...
            >
                <p
                    :style="`background-color: ${
                        totalDayHoursByTasks(resource?.id) > 6
                            ? 'red'
                            : totalDayHoursByTasks(resource?.id) > 0
                            ? 'lightgreen'
                            : ''
                    }; padding: 0rem 1rem;`"
                >
                    {{
                        resource?.id == 0
                            ? ""
                            : totalDayHoursByTasks(resource?.id).toFixed(1)
                    }}
                </p>
            </div>
            <div
            ...
            >
                ...
                >
                    <span
                        ...
                        v-for="(cell, i) in vuecal.timeCells"
                        :key="i"
                        >{{ cell.label }}</span
                    >
                </div>
                <template
                    v-if="isWeekOrDayView && !allDay && specialHours.length"
                >
                    <div
                        class="vuecal__special-hours"
                ...
                <div
                    ...
                    v-if="
                        resource.events.length &&
                        (isWeekOrDayView ||
                            (view.id === 'month' && options.eventsOnMonthView))
                    "
                >
                    <event
                        v-for="(event, j) in resource.events"
                        :key="'event-' - j + '-' + resource.id"
                        :cell-formatted-date="data.formattedDate"
                        @addEvent="$emit('addEvent', $event)"
                        @addEventReverse="$emit('addEventReverse', $event)"
                        @removeEvent="$emit('removeEvent', $event)"
                        @removeEventReverse="
                            $emit('removeEventReverse', $event)
                        "
                        :event="event"
                        :cell="data"
                        :all-day="allDay"
                        ...
                        :overlaps="
                            (
                                (splitsCount
                                    ? split.overlaps[event._eid]
                                    : cellOverlaps[event._eid]) || []
                            ).overlaps
                        "
                        :event-position="
                            (
...
export default {
    ...
    methods: {
        ...
        totalDayHoursByTasks(resourceId) {
            let events =
            ...
            events.forEach((event) => {
                for (let key in event.taskHours) {
                    if (key === this.data.formattedDate) {
                        hours += event.taskHours[key].hours;
                    }
                }
            });
            return hours;
    ...
    computed: {
        ...
        resourcesWithEventsFilteredByViewDates() {
            ...
            this.resources.forEach((resource) => {
                let events = resource.events.filter((event) => {
                    let startDate = event.startDate;
                    let dateExists = false;
                    if (this.matchDates(startDate, this.data.startDate)) {
                        dateExists = true;
                    }
                    if (this.matchDates(startDate, this.data.startDate)) {
                        dateExists = true;
                    ...
                    for (let i = 0; i < resource.events.length; i++)
                        resources.push({
                            ...
                            events: events.filter((event) => {
                                return event.id == resource.events[i].id;
                            }),
                        });
                }
            });
