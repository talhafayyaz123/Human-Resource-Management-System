...
export default {
  ...
  mounted() {
    ...
    this.arrowRightVal = Object.keys(this.event?.taskHours ?? {}).length * 100 - 84;
    ...
    this.$refs.resizerArrowLeft.$el.style.left = -1 * this.arrowLeftVal + "%";
    this.chunkSize =
    ...
    document.addEventListener("mouseup", (e) => {
      ...
      this.oldVal = this.originalOffsetWidth;
      if (this.$refs.vuecalEvent) this.$refs.vuecalEvent.style.left = "0px";
    ...
    document.addEventListener("mousemove", (e) => {
      if (this.isResizingRight) {
        if (this.$refs.resizerArrowRight.$el && this.$refs.vuecalEvent)
          this.$refs.resizerArrowRight.$el.style.right =
            -1 * this.arrowRightVal - (e.clientX - this.dragStartPoint) + "%";
        this.$refs.vuecalEvent.style.width =
        ...
        if (
          ...
          this.$emit("addEvent", { cell: this.cell, event: this.event });
          this.originalOffsetWidth = this.originalOffsetWidth + this.chunkSize;
        } else if (
          this.originalOffsetWidth - this.chunkSize >=
          this.$refs.vuecalEvent.offsetWidth
        ) {
          this.$emit("removeEvent", { cell: this.cell, event: this.event });
          this.originalOffsetWidth = this.originalOffsetWidth - this.chunkSize;
        }
      } else if (this.isResizingLeft) {
        if (this.$refs.vuecalEvent)
          this.$refs.vuecalEvent.style.width =
            this.originalOffsetWidth + (this.dragStartPoint - e.clientX) + "px";
        if (this.$refs.resizerArrowLeft)
          this.$refs.resizerArrowLeft.$el.style.left =
            -1 * (this.arrowLeftVal - (e.clientX - this.dragStartPoint)) + "%";
        if (this.$refs.resizerArrowLeft)
          this.$refs.vuecalEvent.style.left =
            -1 * (this.dragStartPoint - e.clientX) + "px";
        if (
          (-1 * this.$refs.vuecalEvent?.offsetLeft ?? 0) > this.chunkSize &&
          !(
            this.event.startDate.subtractDays(1) <
            new Date().setHours(0, 0, 0, 0)
          )
        ) {
          this.$emit("addEventReverse", { cell: this.cell, event: this.event });
        } else if ((this.$refs.vuecalEvent?.offsetLeft ?? 0) > this.chunkSize) {
  ...
  methods: {
    ...
    onDragStart(e) {
      ...
      this.$refs.vuecalEvent.innerHTML = `<p style="font-size: 0.7rem;">${this.event.startDate.toString().slice(0,10)} - ${this.event.endDate.toString().slice(0,10)}</p>`;
