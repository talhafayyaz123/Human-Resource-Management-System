<template>
    <div style="display: grid; grid-template-columns: 20% 80%">
        <tr
            ...
            class="vl"
        >
            <th class="self-center text-center">Name</th>
        </tr>

        <div
            style="padding: 0"
            class="vuecal__flex vuecal__weekdays-headings flex justify-around"
        >
            <template
                v-for="(heading, i) in view.id === 'year' ? yearHeadings : []"
                :key="i"
            >
                <div
                    ...
                    :class="{
                        today: heading.today,
                        clickable: cellHeadingsClickable,
                    }"
                    :style="weekdayCellStyles"
                    @click="
                        view.id === 'week' && selectCell(heading.date, $event)
                    "
                    @dblclick="
                        view.id === 'week' &&
                ...
                >
                    ...
                    >
                        <div
                            class="vuecal__flex"
                            column="column"
                            :key="vuecal.transitions ? `${i}` : false"
                        >
                            ...
                            <div
                                ...
                                v-if="
                                    vuecal.hasSplits && vuecal.stickySplitLabels
                                "
                                grow="grow"
                            >
                                <div
            ...
            <template
                v-for="(heading, i) in view.id === 'month' ? monthHeadings : []"
                :key="i"
            >
                <div
                    class="vuecal__flex vuecal__heading"
            ...
            <template
                v-for="(heading, i) in view.id === 'week' ? headings : []"
                :key="i"
            >
                <div
                    class="vuecal__flex vuecal__heading"
...
export default {
    ...
    computed: {
        ...
        monthHeadings() {
            ...
            if (
                this.view.startDate.getMonth() ===
                this.utils.date.addDays(this.view.startDate, 28).getMonth()
            ) {
                headings.push({
                    label: "Week 5",
                });
        ...
        headings() {
            ...
            const headings = this.calenderWeekDays.map((cell, i) => {
                const date = this.utils.date.addDays(
                    this.subtractDays(this.view.startDate, 5),
        ...
        cellWidth() {
            return (
                100 /
                (7 - this.calenderWeekDays.reduce((total, day) => total + day.hide, 0))
            );
        },
        weekdayCellStyles() {
            return {
