    ...
    <div>
        ...
        <div class="table-responsive">
            <table class="w-full doc-table">
                ...
                <tr
                    v-for="item in invoiceTypes.data"
                    :key="item.id"
                    ...
                    @contextmenu.stop.prevent="
                        (e) => {
                            ...
                            let route = $router.resolve(
                                `/travel-expenses/invoice-types/${item.id}/edit?page=${page}`
                            );
                ...
                >
                    <td class="border-t">
                        <router-link
                            :to="`/travel-expenses/invoice-types/${item.id}/edit?page=${page}`"
                ...
                <tr v-if="invoiceTypes.data.length === 0">
                    <td class="px-6 py-4 border-t text-center" colspan="4">
                        {{ $t("No invoice type found") }}.
                    </td>
                </tr>
            </table>
        </div>
...
export default {
    ...
    data() {
        return {
            ...
            optionalItems: {
                isBtn2Show: true,
                btn2Text: this.$t("Create Invoice Type"),
                btn2Path: "/travel-expenses/invoice-types/create",
            },
            form: {
                search: "",
                sortBy:
                    localStorage.getItem("sort_invoice_types_column") ?? "name",
                sortOrder:
                    localStorage.getItem("sort_invoice_types_order") ?? "asc",
            },
            window,
        };
    },
