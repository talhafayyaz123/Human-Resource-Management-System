<template>
    <div>
        <h1 class="header mb-8 text-3xl font-bold">
            <router-link class="secondary-color" to="/service-level-agreements">{{
        ...
        <div class="flex justify-center mb-4 border-b border-gray-200 dark:border-gray-700">
            ...
            <div class="flex mr-12">
                <a @click="activeTab = 'serviceTimes'"
                    :class="
        ...
        <div id="myTabContent">
            ...
            >
                ...
                <div class="flex flex-wrap -mb-8 -mr-6 p-8">
                    <text-input
                        ...
                        :required="true"
                    />
                    <div style="text-align: center;" class="pb-8 pr-6 w-full lg:w-1/4">
                        <label for="access">
                            {{ $t("Access") }}
                        </label>
                        <input
                ...
                <div class="bg-white rounded-md shadow overflow-x-auto">
                    <table class="w-full whitespace-nowrap">
                        <tr class="text-left font-bold">
                            ...
                            <th class="pb-4 pt-6 px-6">{{ $t("Included Users") }}</th>
                            <th class="pb-4 pt-6 px-6">{{ $t("Additional Users") }}</th>
                        ...
                        >
                            ...
                            <td class="border-t">
                                <a
                                    ...
                                    class="flex items-center px-6 py-4 focus:text-indigo-500"
                                >
                                {{ $formatter(item?.additionalUser, globalLanguage) }}
                                </a>
                            </td>
                            <td class="w-px border-t">
                                <button
                                @click="popInfrastructure(index)"
                                >
            ...
            >
                <div class="flex flex-wrap -mb-8 -mr-6 p-8">
                    ...
                    >
                        <option v-for="option in slaInfrastructures" :value="option.infrastructureName" :key="option.infrastructureName">
                        ...
                        >
                            ...
                            <td class="border-t">
                                <a
                                    ...
                                    class="flex items-center px-6 py-4 focus:text-indigo-500"
                                >
                                <span class="mr-3 success-badge" v-for="(day,index) in item?.days" :key="index">
            ...
            <div v-for="(levelDetail, index) in levelDetails"  class="flex flex-wrap -mb-8 -mr-6 p-8" :key="index">
                ...
                <SelectInput
                    ...
                    >
                    <option v-for="option in priorityOptions" :value="option.id" :key="option.id">
                        {{ option.name }}
                    </option>
                </SelectInput>
                <text-input
                ...
                <text-input
                    ...
                    :label="$t('Change')"
                />
                <div role="button" class="float-center" @click="deletelevelDetails(index)">
            ...
            <div class="flex flex-wrap -mb-8 -mr-6 p-8">
                ...
                <span @click="addLine" class="ms-2 font-weight-medium text-primary">
                    <span class="fas fa-plus-circle text-info"></span>
                    {{ $t("Add Line") }}
                </span>
            </div>
            <span class="error" v-if="errorSlaLevel">{{ errorSlaLevel }}</span>
                <div
                    class="flex items-center justify-end px-8 py-4 bg-gray-50 border-t border-gray-100"
                >
                    <loading-button
                        ...
                        >
                            ...
                            <td class="border-t">
                                <a
                                    ...
                                    class="flex items-center px-6 py-4 focus:text-indigo-500"
                                >
                                <span class="mr-3" v-for="(level,index) in item?.levels" :key="index">
        ...
        >
            <loading-button
                v-if="$can(`${$route.meta.permission}.create`)"
                @click="updateServiceLevelAgreement()"
                :loading="isLoading"
                class="secondary-btn"
                >{{ $t("Update") }}
            </loading-button>
    ...
    methods: {
        async updateServiceLevelAgreement(){
           ...
           const payload = {
            ...
            this.$store.commit("isLoading", true);
            await this.$store.dispatch("serviceLevelAgreements/update", {
        ...
        async refresh() {
            try {
                const response = await this.$store.dispatch(
                    "serviceLevelAgreements/show",
                    this.$route.params.id
                );
                this.serviceLevelAgreement = response?.data?.serviceLevelAgreement ?? {};
                this.slaInfrastructures = this.serviceLevelAgreement?.slaInfrastructure 
                this.slaServiceTimes = this.serviceLevelAgreement?.slaService 
                this.slaLevels = this.serviceLevelAgreement?.slaLevel 
                this.form = {
                    name: this.serviceLevelAgreement?.name,
                };
            } catch (e) {}
        },
        addLine(){
