...
export default {
    ...
    computed: {
        ...
        series() {
            const diskStats = this.statistics
                .sort(function (a, b) {
                    return a.timestamp - b.timestamp;
                })
                .flatMap((stat) => stat.data.disk.disks)
                .reduce((acc, disk) => {
                    const { filesystem, used_percentage } = disk;
                    acc[filesystem] = acc[filesystem] || [];
                    acc[filesystem].push(used_percentage);
                    return acc;
                }, {});

            return Object.entries(diskStats).map(([name, data]) => ({
                name,
                data,
            }));
        },
