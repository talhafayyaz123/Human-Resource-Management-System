            ...
            <table class="w-full doc-table">
                ...
                <tr
                    ...
                    @contextmenu.stop.prevent="
                        (e) => {
                            e.preventDefault();
                            let route = $router.resolve(`/product-unit/${unit.id}/edit?page=${page}`);
                ...
                >
                    ...
                    <td class="w-px border-t">
                        <router-link
                            ...
                            class="mr-2"
                            :to="`/product-unit/${unit.id}/edit?page=${page}`"
                        >
                            <font-awesome-icon
...
export default {
    ...
    data() {
        return {
            ...
            optionalItems: {
                isBtn2Show: true,
                btn2Text: this.$t("Create Units"),
                btn2Path: "/product-unit/create",
            },
            form: {
                search: "",
                sortBy: "id",
    ...
    methods: {
        ...
        async refresh(bypass) {
            if (!this.units?.data.length || bypass)
                await this.$store.dispatch("units/list", {
                    page: this.page,
                    search: this.form.search,
                });
        ...
        async destroy(id) {
            this.$swal({
                ...
                if (result.isConfirmed === true) {
                    try {
                        await this.$store.dispatch("units/destroy", id);
                        this.refresh(true);
                    } catch (e) {}
                }
            });
