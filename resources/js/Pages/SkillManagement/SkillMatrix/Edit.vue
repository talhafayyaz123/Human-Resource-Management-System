<template>
    <div>
        ...
        <div class="bg-white rounded-md shadow margin-bottom-3rem">
            <div class="flex justify-start 4">
                <button @click="preview()" class="secondary-btn">
                    {{ $t("Preview Matrix Update") }}
                </button>
            </div>
            <div class="matrix p-5" v-if="previewMatrix">
    ...
    methods: {
        async refresh() {
            try {
                ...
                document.title = "Edit Skill Matrix " + this.form?.name;
                await this.showDefaultOptions();
                await this.preview();
            } catch (e) {
            } finally {
        ...
        async showDefaultOptions() {
            if (Array.isArray(this.form.skillGroup)) {
                this.form.skillGroup.map((group) => {
                    let obj = this.skillGroup?.data.find(
                        (option) => option.id === group.id
                    );
                    if (!!obj) {
                        this.removeSkills(obj);
                    }
                });
            }

            if (Array.isArray(this.form.teams)) {
                this.form.teams.map((team) => {
                    let obj = this.teams?.data.find(
                        (option) => option.id === team.id
                    );
                    if (!!obj) {
                        this.removeTeams(obj);
                    }
                });
            }
        },
        ...
        async update() {
            try {
                ...
                await this.$store.dispatch("skillsMatrix/update", {
                ...
                this.$router.push(
                    `/skill-matrix?page=${this.$route.query.page}`
                );
            } catch (e) {}
        },
