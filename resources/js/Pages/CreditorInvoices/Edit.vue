    ...
    <div>
        ...
        <form @submit.prevent="store">
            ...
            <div class="flex items-center justify-end mt-4">
                <router-link
                    :to="`/creditor-invoices?page=${this.$route.query.page}`"
    ...
    async mounted() {
        try {
            ...
            this.form = {
                ...
                invoicePeriod: response?.data?.data?.invoicePeriod ?? "",
                termsOfPayment: response?.data?.data?.termsOfPayment ?? "",
                customNotesField: response?.data?.data?.customNotesField ?? "",
                contactPerson: response?.data?.data?.contactPerson ?? "",
                draftStatusChangeDate: new Date(
                    response?.data?.data?.draftStatusChangeDate ?? ""
                ),
                totalAmount: response?.data?.data?.totalAmount ?? 0,
                totalTaxAmount: response?.data?.data?.totalTaxAmount ?? 0,
                totalAmountWithoutTax:
                    response?.data?.data?.totalAmountWithoutTax ?? 0,
            };
            document.title =
                "Edit Creditor Invoice " + response?.data?.data?.invoiceType ??
                "";
            if (response?.data?.data?.contactPerson)
                this.$store
                    .dispatch("auth/show", {
                        id: response?.data?.data?.contactPerson,
                    })
                    .then((res) => {
    ...
    methods: {
        ...
        async update() {
            ...
            try {
                ...
                await this.$store.dispatch("creditorInvoices/update", {
                ...
                this.$router.push(
                    "/creditor-invoices?page=" + this.$route.query.page
                );
            } catch (e) {
                console.log(e);
                this.disabled = false;
