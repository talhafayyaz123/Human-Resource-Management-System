    ...
    <div>
        ...
        <div class="table-responsive">
            <table class="w-full doc-table">
                ...
                <tr
                    v-for="group in affectedGroups?.data"
                    :key="group.id"
                    class="hover:bg-gray-100 focus-within:bg-gray-100"
                    @contextmenu.stop.prevent="
                        (e) => {
                            e.preventDefault();
                            let route = $router.resolve(`/affected-groups/${group.id}/edit?page=${page}`);
                ...
                >
                    ...
                    <td class="w-px border-t">
                        <router-link
                            ...
                            class="mr-2"
                            :to="`/affected-groups/${group.id}/edit?page=${page}`"
                        >
                            <font-awesome-icon
                ...
                <tr v-if="affectedGroups?.data.length === 0">
                    <td class="px-6 py-4 border-t text-center" colspan="4">
                        {{ $t("No affected groups found") }}.
                    </td>
                </tr>
            </table>
        </div>
...
export default {
    ...
    data() {
        return {
            ...
            optionalItems: {
                isBtn2Show: true,
                btn2Text: this.$t("Create Affected Group"),
                btn2Path: "/affected-groups/create",
            },
            page: 1,
            form: {
                search: "",
    ...
    methods: {
        async refresh(bypass) {
            if (!this.affectedGroups?.data.length || bypass)
        ...
        async destroy(id) {
            this.$swal({
                ...
                if (result.isConfirmed === true) {
                    try {
                        await this.$store.dispatch("affectedGroups/destroy", id);
                        this.refresh(true);
                    } catch (e) {}
                }
            });
