    ...
    <div>
        ...
        <form @submit.prevent="store">
            ...
            <div class="card mt-3">
                ...
                <div class="card-body">
                    <CronExpressionMaker
                        :form-reset-pattern="form.reset_pattern"
    ...
    async mounted() {
        ...
        try {
            ...
            this.form = {
                ...
                max_value: response?.data?.max_value ?? 0,
                min_value: response?.data?.min_value ?? 0,
                value: response?.data?.value ?? 0,
                deny_if_value: response?.data?.deny_if_value ?? 0,
                role_id: response?.data?.role_id ?? "",
                config: response?.data?.config ?? "",
                reset_type: response?.data?.reset_type ?? "",
                reset_value: response?.data?.reset_value ?? "",
                reset_frequency_hours:
                    response?.data?.reset_frequency_hours ?? 0,
                reset_pattern: response?.data?.reset_pattern ?? "",
                deny_if_op: response?.data?.deny_if_op ?? "",
                status: response?.data?.status ?? 0,
            };
            if (!this.roles?.length) await this.$store.dispatch("roles/list");
            this.form.role_id =
                this.roles.find((role) => role.id == this.form.role_id) ?? {};
            if (response?.data?.event_name) {
                const eventName = await this.$store.dispatch("eventName/show", {
                    id: response?.data?.event_name,
                });
                this.form.event_name = eventName?.data?.[0] ?? "";
            }
    ...
    methods: {
        ...
        async store() {
            try {
                ...
                await this.$store.dispatch("rules/update", {
                    ...this.form,
