            ...
            <table class="doc-table">
                <thead>
                    <tr class="text-left font-bold">
                        <th
                            @click="sort('companyNumber', 'customers')"
                        ...
                        <th style="width: 240px"
                            @click="sort('companyName', 'customers')"
                            class="cursor-pointer"
                        >
                            {{ $t("Customer Name")
                            }}<font-awesome-icon
                        ...
                        <th style="width: 140px"
                            @click="sort('customerType', 'customers')"
                            class="cursor-pointer"
                        >
                            {{ $t("Type")
                            }}<font-awesome-icon
                ...
                <tbody>
                    ...
                    <tr
                        v-for="customer in Partnercompanies?.data ?? []"
                        :key="customer.id"
                        ...
                        <td class="w-px text-center">
                            <div class="flex items-center gap-3">
                                <router-link
                                    ...
                                    "
                                    :to="`/partner/companies/${customer.id}?page=${page}`"
                                >
                                ...
                                <a
                                    ...
                                    @click.stop="
                                        $router.push(
                                            `/partner/companies/${customer.id}/edit?page=${page}`
                                        )
                                    "
                                >
                                    <font-awesome-icon
                    ...
                    <tr v-if="(Partnercompanies?.data?.length ?? 0) === 0">
                        <td class="" colspan="4">
...
export default {
    computed: {
        ...mapGetters("companies", ['Partnercompanies']),
    },
    mounted() {
        this.refresh();
    },
    ...
    data() {
        return {
            ...
            optionalItems: {
                ...
                isBtn2Show: false,
                btn2Text: this.$t("Create Customer"),
                btn2Path: "",
            },
            window,
            form: {
    ...
    methods: {
        async changePageOrSearch(page = 1) {
            this.page = page;
            await this.$store.dispatch("companies/PartnerCompanylist", {
                page: page,
        ...
        async destroy(id) {
            this.$swal({
                ...
                if (result.isConfirmed === true) {
                    await this.$store.dispatch("companies/partnerCompanyDestroy", id);
                    this.refresh();
                }
            });
        },
