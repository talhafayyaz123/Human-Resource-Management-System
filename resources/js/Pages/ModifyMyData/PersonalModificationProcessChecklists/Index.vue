            ...
            <table class="w-full doc-table">
                ...
                <tr
                    ...
                    @contextmenu.stop.prevent="
                        (e) => {
                            e.preventDefault();
                            let route = $router.resolve(`/personal-modification-process-checklists/${item.id}/edit?page=${page}`);
                ...
                >
                    ...
                    <td class="w-px border-t">
                        <router-link
                            ...
                            class="px-1 mr-2"
                            :to="`/personal-modification-process-checklists/${item.id}/edit?page=${page}`"
                        >
                            <font-awesome-icon
                ...
                <tr v-if="(checklists?.data?.length ?? 0) === 0">
                    <td class="px-6 py-4 border-t text-center" colspan="4">
                        {{ $t("No checklists found") }}.
                    </td>
                </tr>
            </table>
        </div>
...
export default {
    computed: {
        ...mapGetters("personalModificationProcessChecklists", ["checklists"]),
    },
    components: {
        Pagination,
        SearchFilter,
    ...
    data() {
        return {
            ...
            optionalItems: {
                isBtn2Show: true,
                btn2Text: this.$t("Create Checklist"),
                btn2Path: "/personal-modification-process-checklists/create",
            },
            page: 1,
            form: {
                search: "",
    ...
    methods: {
        ...
        async refresh(bypass) {
            if (!this.checklists?.data?.length || bypass)
                await this.$store.dispatch(
                    "personalModificationProcessChecklists/list",
                    {
                        page: this.page,
        ...
        async destroy(id) {
            this.$swal({
                ...
                if (result.isConfirmed === true) {
                    try {
                        await this.$store.dispatch(
                            "personalModificationProcessChecklists/destroy",
                            id
                        );
                        this.refresh(true);
                    } catch (e) {}
