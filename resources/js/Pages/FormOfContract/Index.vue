    ...
    <div>
        ...
        <div class="table-responsive">
            <table class="w-full doc-table">
                ...
                <tr
                    v-for="contract in formOfContract.data"
                    :key="contract.id"
                    class="hover:bg-gray-100 focus-within:bg-gray-100"
                    @contextmenu.stop.prevent="
                        (e) => {
                            e.preventDefault();
                            let route = $router.resolve(`/form-of-contract/${contract.id}/edit?page=${page}`);
                ...
                >
                    ...
                    <td class="w-px border-t">
                        <router-link
                            ...
                            class="px-1 mr-2"
                            :to="`/form-of-contract/${contract.id}/edit?page=${page}`"
                        >
                            <font-awesome-icon
                ...
                <tr v-if="formOfContract.data.length === 0">
                    <td class="px-6 py-4 border-t text-center" colspan="4">
                        {{ $t("No Form Of Contract found") }}.
                    </td>
                </tr>
            </table>
        </div>
...
export default {
    ...
    data() {
        return {
            ...
            optionalItems: {
                isBtn2Show: true,
                btn2Text: this.$t("Create Form Of Contract"),
                btn2Path: "/form-of-contract/create",
            },
            page: 1,
            form: {
                search: "",
   ...
   watch: {
    ...
    methods: {
        /**
         * get the form of contract list based on page that was selected with pagination
        ...
        async refresh(bypass) {
            if (!this.formOfContract.data.length || bypass)
                await this.$store.dispatch("formOfContract/list", {
                    page: this.page,
                });
        },
        async destroy(id) {
            this.$swal({
                ...
                if (result.isConfirmed === true) {
                    try {
                        await this.$store.dispatch("formOfContract/destroy", id);
                        this.refresh(true);
                    } catch (e) {}
                }
            });
