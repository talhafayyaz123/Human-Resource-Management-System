        ...
        <div class="table-responsive">
            <table class="w-full doc-table">
                <tr class="text-left font-bold">
                    ...
                    <th
                        @click="sort('installName')"
                        class="pb-4 pt-6 px-6 cursor-pointer"
                    >
                        {{ $t("install Name") }}
                        <font-awesome-icon
                            v-if="form.sortBy === 'install_name'"
                            class="cursor-pointer ml-2"
                            :icon="`fa-solid fa-sort-${
                                form.sortOrder === 'asc' ? 'up' : 'down'
                            }`"
                ...
                <tr
                    ...
                    @contextmenu.stop.prevent="
                        (e) => {
                            e.preventDefault();
                            let route = $router.resolve(`/elo-business-solution/${host.id}/edit?page=${page}`);
                ...
                >
                    ...
                    <td class="w-px border-t">
                        <router-link
                            ...
                            class="px-1 mr-2"
                            :to="`/infrastructure-settings/${host.id}/edit?page=${page}`"
                        >
                            <font-awesome-icon
                ...
                <tr v-if="hosts?.data?.length === 0">
                    <td class="px-6 py-4 border-t" colspan="4">
                        {{ $t("No Infrastructure Settings Record found") }}.
                    </td>
                </tr>
            </table>
        </div>
...
export default {
    ...
    data() {
        return {
            ...
            optionalItems: {
                isBtn2Show: true,
                btn2Text: this.$t("Create Infrastructure Settings"),
                btn2Path: "/infrastructure-settings/create",
            },
            page: 1,
            form: {
                search: "",
        ...
        async destroy(id) {
            this.$swal({
                ...
                if (result.isConfirmed === true) {
                    try {
                        await this.$store.dispatch("eloBusinessSolutions/destroy", id);
                        this.refresh(true);
                    } catch (e) {}
                }
            });
