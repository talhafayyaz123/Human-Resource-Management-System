            ...
            <table class="doc-table">
                <thead>
                    <tr class="text-left">
                        ...
                        <th
                            class=" cursor-pointer"
                            @click="sort('departmentHead', 'departments')"
                        >
                            {{ $t("Department Head") }}
                            <font-awesome-icon
                                v-if="form.sortBy === 'departmentHead'"
                                class="cursor-pointer ml-2"
                                :icon="`fa-solid fa-sort-${
                                    form.sortOrder === 'asc' ? 'up' : 'down'
                                }`"
                    ...
                    >
                        ...
                        <td class="w-px ">
                            <router-link
                                ...
                                :to="`/user/departments/${item.id}/edit`"
                                class="px-2"
                            >
                                <font-awesome-icon
...
export default {
    ...
    data() {
        return {
            ...
            optionalItems: {
                isBtn2Show: true,
                btn2Text: this.$t("Create Department"),
                btn2Path: "/user/departments/create",
            },
            page: 1,
            form: {
                search: "",
                sortBy:
                    localStorage.getItem("sort_departments_column") ?? "name",
                sortOrder:
                    localStorage.getItem("sort_departments_order") ?? "asc",
            },
            window,
        };
    },
        ...
        async destroy(id) {
            this.$swal({
                ...
                if (result.isConfirmed === true) {
                    await this.$store.dispatch("userDepartments/destroy", id);
                    this.refresh(true);
                }
            });
        },
