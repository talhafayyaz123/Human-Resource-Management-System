    ...
    <div>
        ...
        <div class="table-responsive">
            <table class="w-full doc-table">
                ...
                <tr
                    v-for="type in protocolTypes?.data"
                    :key="type.id"
                    ...
                    @contextmenu.stop.prevent="
                        (e) => {
                            ...
                            let route = $router.resolve(
                                `/protocol-types/${type.id}/edit?page=${page}`
                            );
                ...
                >
                    ...
                    <td class="w-px border-t">
                        <router-link
                            ...
                            class="mr-2"
                            :to="`/protocol-types/${type.id}/edit?page=${page}`"
                        >
                            <font-awesome-icon
                ...
                <tr v-if="protocolTypes?.data.length === 0">
                    <td class="px-6 py-4 border-t text-center" colspan="4">
                        {{ $t("No protocol types found") }}.
                    </td>
                </tr>
            </table>
        </div>
...
export default {
    ...
    data() {
        return {
            ...
            optionalItems: {
                isBtn2Show: true,
                btn2Text: this.$t("Create Protocol Type"),
                btn2Path: "/protocol-types/create",
            },
            page: 1,
            form: {
                search: "",
    ...
    methods: {
        async refresh(bypass) {
            if (!this.projectStatuses?.data.length || bypass)
        ...
        async destroy(id) {
            this.$swal({
                ...
                if (result.isConfirmed === true) {
                    try {
                        await this.$store.dispatch("protocolTypes/destroy", id);
                        this.refresh(true);
                    } catch (e) {}
                }
            });
