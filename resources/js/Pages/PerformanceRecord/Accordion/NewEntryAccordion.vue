<template>
    <button
        @click="toggleAccordion('newEntry')"
        class="accordion-button rounded-md shadow"
    >
        <span
            class="accordion-header-icon"
            :class="{
                'accordion-header-icon--open': isAccordionOpen('newEntry'),
            }"
        ></span>
        ...
        async recordOrderChanged(event) {
            if (event.moved) {
                try {
                    ...
                    const movedEntry = this.newEntry[movedIndex];
                    const previousEntry = this.newEntry[movedIndex - 1];
                    const nextEntry = this.newEntry[movedIndex + 1];
