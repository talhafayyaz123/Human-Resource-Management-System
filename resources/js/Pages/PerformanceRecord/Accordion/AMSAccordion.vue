<template>
    <button
        @click="toggleAccordion('ams')"
        class="accordion-button rounded-md shadow"
    >
        <span
            class="accordion-header-icon"
            :class="{
                'accordion-header-icon--open': isAccordionOpen('ams'),
            }"
        ></span>
        <span class="accordion-header-text secondary-color">{{
            $t("Application Management Service")
        }}</span>
    </button>
    <div :class="{ 'accordion-content': true }">
        <table class="w-full whitespace-nowrap">
            ...
            <draggable
                ...
                tag="tbody"
            >
                <template v-for="entry in ams" v-slot:item="{ element: entry }">
                    <tr
                        :class="
                    ...
                    >
                        ...
                        <td>{{ replace(entry.quantity) }} {{ $t("Hours") }}</td>
                        <td>
        ...
        async recordOrderChanged(event) {
            if (event.moved) {
                try {
                    ...
                    const movedEntry = this.ams[movedIndex];
                    const previousEntry = this.ams[movedIndex - 1];
                    const nextEntry = this.ams[movedIndex + 1];
