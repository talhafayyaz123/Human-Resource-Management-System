...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('product_childrens', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('child_version_id');
            $table->timestamps();
            //$table->unique(['product_version_id', 'child_version_id'], 'unique_versions');
            $table->foreign('product_version_id')->references('id')->on('product_versions')->onDelete('cascade');
            $table->foreign('child_version_id')->references('id')->on('product_versions')->onDelete('cascade');
        });
    }
